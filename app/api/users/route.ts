...
export async function GET(request: NextRequest) {
  try {
    ...
    if (userId) {
      const user = await db
        .prepare(`
          SELECT id, username, email, role, is_banned, is_unlimited, 
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { userId, isBanned, isUnlimited } = body;
    ...
    if (typeof isBanned === 'number') {
      ...
      bindings.push(isBanned ? 1 : 0);
    }

    if (typeof isUnlimited === 'number') {
      updates.push("is_unlimited = ?");
      bindings.push(isUnlimited ? 1 : 0);
    }
    ...
    return NextResponse.json({
      message: "用户状态更新成功",
      data: { userId, isBanned, isUnlimited },
    });
  } catch (error) {
